...
(echo "$1" | awk "/$2/,/\)/" | grep "DKIM" > /dev/null) && (echo "$1" | awk "/_domainkey.$2/,/\)/" | grep -v "key#" | grep -v "^$"  |\
 sed "s/  \"\|\"//g" | awk '$1=$1' ORS='' | sed "s/(/ \
 \&nbsp;<br\/>\<input class\=\"style99\" size\=\"100\" disabled\=\"disabled\" type\=\"text\" value\=\"/g" | sed "s/)/\"\>/g") || \
 printf "<b>DKIM signature<br/><input class=\"style99\" size=\"100\" disabled=\"disabled\" type=\"text\" value=\"$mess\" /></b>";
 ;;
esac;
